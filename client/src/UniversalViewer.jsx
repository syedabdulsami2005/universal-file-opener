...
const UniversalViewer = ({ file, fileType, fileContent, backendData }) => {
  ...
  const renderContent = (type, url, content, data, tableData, fileName) => {
    ...
    // C. IMAGES (Zoom Enabled - FIXED: Fits screen initially)
    if (EXT_MAP.image.includes(type) || data?.type === 'image_pass') {
      return (
        <ZoomWrapper>
          <div className="flex items-center justify-center min-h-full">
             <img src={url} className="max-w-full max-h-full object-contain shadow-md" />
          </div>
        </ZoomWrapper>
      );
    }

    // D. PDF (Zoom Wrapper Enabled + White BG)
    ...
    // E. CODE (FIXED: Absolute positioning to force full screen + Native Zoom)
  ...
  if (selectedZipFile) {
    return (
      <div className="flex flex-col h-full bg-gray-100">
        ...
        <div className="flex-1 overflow-hidden relative w-full h-full"> {/* FIXED: Changed to h-full to support absolute child */}
